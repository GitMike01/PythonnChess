...
class Chessboard():
    ...
    def chessboard_whites_init(self):
        ...
        for i in range(self.size):
            ...
            self.board[1, i] = PawnPiece(1, i, PieceColor.WHITE, image=pawn_img)
            piece_class = row_0_layout[i]
            piece_type = row_0_types[i]
            self.board[0, i] = piece_class(0, i, PieceColor.WHITE, image=self.res.sprites[PieceColor.WHITE][piece_type])
    ...
    def chessboard_blacks_init(self):
        ...
        for i in range(self.size):
            ...
            self.board[6, i] = PawnPiece(6, i, PieceColor.BLACK, image=pawn_img)
            piece_class = row_7_layout[i]
            piece_type = row_6_types[i]
            self.board[7, i] = piece_class(7, i, PieceColor.BLACK, image=self.res.sprites[PieceColor.BLACK][piece_type])
